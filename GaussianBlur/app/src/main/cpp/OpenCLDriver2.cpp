...
JNIEXPORT jobject
JNICALL
Java_com_example_gaussianblur_MainActivity_GaussianBlurBitmap(JNIEnv *env, jobject thiz, jobject bitmap) {
...
Java_com_example_gaussianblur_MainActivity_GaussianBlurGPU(JNIEnv *env, jobject thiz, jobject bitmap) {
    AndroidBitmapInfo info;
    int ret;
    if ((ret = AndroidBitmap_getInfo(env, bitmap, &info)) < 0) {
        LOGE("AndroidBitmap_getInfo() failed! error=%d", ret);
