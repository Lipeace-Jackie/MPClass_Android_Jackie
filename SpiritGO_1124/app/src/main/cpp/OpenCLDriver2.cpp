...
JNIEXPORT jobject
extern "C" JNICALL
Java_com_example_spiritgo_11124_MainActivity_GaussianBlurBitmap(JNIEnv *env, jobject thiz, jobject bitmap) {
...
Java_com_example_spiritgo_11124_MainActivity_GaussianBlurGPU(JNIEnv *env, jobject thiz, jobject bitmap) {
    AndroidBitmapInfo info;
    int ret;
    if ((ret = AndroidBitmap_getInfo(env, bitmap, &info)) < 0) {
        LOGE("AndroidBitmap_getInfo() failed! error=%d", ret);
    ...
    char *kernel_name = "kernel_blur_RGBA";
    int pixelsCount = info.height * info.width;
